	...
	<div class="layui-fluid">
		<div class="layui-row">
			<form class="layui-form">
				<div class="layui-form-item">
					...
					<div class="layui-input-inline">
						<input type="text" id="L_id" name="id" required=""
							placeholder="请输入部门编号" lay-verify="id" value="${dept.id }"
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-inline">
						<input type="text" id="L_name" name="name" required=""
							value="${dept.name }" placeholder="请输入部门名称" lay-verify="name"
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-inline">
						<input type="text" id="L_phone" name="phone" required=""
							value="${dept.phone }" placeholder="请输入移动电话" lay-verify="phone"
							autocomplete="off" class="layui-input">
					</div>
				</div>
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-inline">
						<input type="text" id="L_tel" name="tel" required=""
							value="${dept.tel }" placeholder="请输入部门电话" lay-verify="tel"
							autocomplete="off" class="layui-input">
					</div>
				</div>

				<div class="layui-form-item">
					<label for="L_tel" class="layui-form-label">部门负责人 </label>
					<div class="layui-input-block">
						<input type="radio" name="user.id" value="0" title="无"
							checked="checked" />
						<c:forEach items="${users }" var="user">

							<c:if test="${dept.user.id eq user.id }">
								<input type="radio" name="user.id" value="${user.id }"
									title="${user.name }" checked="checked" />
							</c:if>
							<c:if test="${dept.user.id ne user.id }">
								<input type="radio" name="user.id" value="${user.id }"
									title="${user.name }" />
							</c:if>
						</c:forEach>
				...
				<div class="layui-form-item">
					<label for="L_tel" class="layui-form-label">负责机构 </label>
					<div class="layui-input-block">
						<input type="radio" name="branch.id" value="0" title="无"
							checked="checked" />
						<c:forEach items="${branchs }" var="branch">

							<c:if test="${dept.branch.id eq branch.id }">
								<input type="radio" name="branch.id" value="${branch.id }"
									title="${branch.name }" checked="checked" />
							</c:if>
							<c:if test="${dept.branch.id ne branch.id }">
								<input type="radio" name="branch.id" value="${branch.id }"
									title="${branch.name }" />
							</c:if>
						</c:forEach>
